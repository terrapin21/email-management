...
【担当者選択ルール】書類に「販売店担当者」と「施工店担当者」の両欄がある場合、監督名・監督連絡先（電話（携帯））は以下の優先順位で選択してください：
1. 氏名が記入されている方を選ぶ
2. 両方に氏名がある場合は「電話（携帯）」が記入されている方を選ぶ
3. 両方に氏名・電話（携帯）がある場合は「施工店担当者」を選ぶ
監督連絡先には「電話（携帯）」の値を設定してください（「電話（会社）」は使わない）。
例: {example}

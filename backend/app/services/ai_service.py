...
- 例：「施主コード：A-1234」「回収品目：段ボール」「回収日：4月25日」「連絡先：03-1234-5678」
- 項目名と値のペアをすべて extracted_fields に含めてください
- 明確に記載されている場合のみ抽出し、推測はしないでください
- 【重要】施主No・施主コード・工事番号・現場コード・発注No・案件No・物件コードなど現場を特定する固有IDは、値にIDと名称（〇〇様邸・〇〇邸・〇〇様など）が続けて記載されていても**ID部分のみ**を抽出してください。例：「AABBCC〇〇様邸」→「AABBCC」、「A-1234田中様邸」→「A-1234」（メーカーによって呼び名が異なりますが、英数字やハイフンで構成される固有コード部分のみが対象です）
- 【重要】同じ種別の現場コード（施主No等）が本文中に複数存在する場合は、**1つのキーにまとめてカンマ区切り**で値を設定してください。例：「ABCD,EFGH,IJKL」
